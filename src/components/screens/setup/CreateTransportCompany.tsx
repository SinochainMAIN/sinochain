...
import { TypeTransportCompanyForm } from '@/types/company.types'
...
function CreateTransportCompany() {
	...
	const {
		...
		control,
		register
	} = useForm<TypeTransportCompanyForm>({
		mode: 'onSubmit',
		...
		defaultValues: {
			...
			customs: false,
			yeo: false,
			expensive_cargo: false,
			transportation_directions: []
		}
	})

	const onSubmit = (data: TypeTransportCompanyForm) => {
		console.log(data)
	}
					...
					render={({ field: { value, onChange } }) => {
						return (
							<Checkbox
								checked={value as boolean}
								onChange={onChange}
								label={t('setup.company.yeo')}
							/>
						)
					}}
				/>
				<Controller
					name='expensive_cargo'
					control={control}
					render={({ field: { value, onChange } }) => {
						return (
							<Checkbox
								checked={value as boolean}
								onChange={onChange}
								label={t('setup.company.expensive_cargo')}
							/>
						)
					}}
				/>
